...
struct ufixed_latlng {
  ...
  static std::uint32_t lng_double_to_ufixed(double const lng) noexcept {
    return static_cast<std::uint32_t>(
        std::round(shift_prime_meridian(lng) * kCoordinatePrecision));
  }

  explicit ufixed_latlng(latlng const& c)
      : lat_{lat_double_to_ufixed(c.lat_)},
        lng_{lng_double_to_ufixed(c.lng_)} {}
