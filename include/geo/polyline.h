...
polyline simplify(polyline const&, double const);

template <int TileSize, int MaxZoomLevel = 20>
polyline simplify(polyline const& polyline, uint32_t const z) {
  constexpr auto kMinPixel = 5;
  ...
  return simplify(polyline, lut.values[z]);
}
