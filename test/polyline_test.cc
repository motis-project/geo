...
TEST_CASE("polylineDistanceToPolyline_pointBeforeLine_getStartPoint") {
  ...
  CHECK(projection.distance_to_polyline_ ==
...
TEST_CASE("polylineDistanceToPolyline_pointAfterLine_getEndPoint") {
  ...
  CHECK(projection.distance_to_polyline_ ==
        distance(test_point, end_coordinate));
...
TEST_CASE("polylineDistanceToPolyline_pointNotOnLine_getClosestPoint") {
  ...
  CHECK(abs(distance(test_point, best) -
            distance(test_point, projection.best_)) < kEpsilon);
  CHECK(abs(projection.distance_to_polyline_ - distance(test_point, best)) <
        kEpsilon);
