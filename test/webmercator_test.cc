...
TEST_CASE("latlng to pixel project pixel") {
  ...
  SUBCASE("projects a center point") {
  ...
  SUBCASE("projects the northeast corner of the world") {
  ...
  SUBCASE("projects the southwest corner of the world") {
    auto const merc = geo::latlng_to_merc({-geo::kMercMaxLatitude, -180});
...
TEST_CASE("pixel to latlng unproject pixel") {
  ...
  SUBCASE("reprojects a center point") {
    auto const pos = geo::merc_to_latlng({merc256::pixel_to_merc_x(128, 0),  //
                                          merc256::pixel_to_merc_y(128, 0)});
    CHECK(pos.lat_ == doctest::Approx(0.));
    CHECK(pos.lng_ == doctest::Approx(0.));
  }

  SUBCASE("reprojects the northeast corner of the world") {
    auto const pos = geo::merc_to_latlng({merc256::pixel_to_merc_x(256, 0),  //
                                          merc256::pixel_to_merc_y(0, 0)});
    CHECK(pos.lat_ == doctest::Approx(geo::kMercMaxLatitude));
    CHECK(pos.lng_ == doctest::Approx(180.));
  }

  SUBCASE("reprojects the southwest corner of the world") {
...
TEST_CASE("latlng to merc project merc") {
  constexpr auto kMercWorldLimit = geo::kMercOriginShift;
  {
    ...
    CHECK(merc.x_ == doctest::Approx(3339584.7238));
    CHECK(merc.y_ == doctest::Approx(6446275.84102));
  }
  {
    auto const merc = geo::latlng_to_merc({geo::kMercMaxLatitude, 180.});
    CHECK(merc.x_ == doctest::Approx(kMercWorldLimit));
    CHECK(merc.y_ == doctest::Approx(kMercWorldLimit));
  }
  {
    auto const merc = geo::latlng_to_merc({-geo::kMercMaxLatitude, -180.});
    CHECK(merc.x_ == doctest::Approx(-kMercWorldLimit));
...
TEST_CASE("merc to latlng unproject merc") {
  constexpr auto kMercWorldLimit = geo::kMercOriginShift;
  {
    ...
    CHECK(pos.lat_ == doctest::Approx(50.));
    CHECK(pos.lng_ == doctest::Approx(30.));
  }
  {
...
TEST_CASE("webmercator reversible") {
  ...
  auto const test = [](auto const& input, auto const z) {
    ...
    CHECK(merc_a.x_ == doctest::Approx(merc_b.x_));
    CHECK(merc_a.y_ == doctest::Approx(merc_b.y_));
    CHECK(input.x_ == doctest::Approx(output_a.x_).epsilon(1));
    CHECK(input.y_ == doctest::Approx(output_a.y_).epsilon(1));
    CHECK(input.x_ == doctest::Approx(output_b.x_).epsilon(1));
    CHECK(input.y_ == doctest::Approx(output_b.y_).epsilon(1));
  };
